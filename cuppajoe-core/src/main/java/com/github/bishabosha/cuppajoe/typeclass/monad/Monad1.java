...
public interface Monad1<INSTANCE extends Monad1, T> extends Applicative1<INSTANCE, T> {
    ...
    <U> Monad1<INSTANCE, U> flatMap(@NonNull Function<? super T, Monad1<INSTANCE, ? extends U>> mapper);

    default <U> Monad1<INSTANCE, U> bless(U value) {
        return Type.lA(pure(value));
    }

    static <X, R, INSTANCE extends Monad1, RA extends Monad1<INSTANCE, R>> RA applyImpl(@NonNull Monad1<INSTANCE, X> m,
                                                                                        @NonNull Applicative1<INSTANCE, Function<? super X, ? extends R>> applicative) {
        Objects.requireNonNull(applicative, "applicative");
        return Type.narrow(Type.lA(applicative).flatMap(xM -> Type.<INSTANCE, R>lF(m.map(xM::apply))));
    }

    static <U, INSTANCE extends Monad1, FINAL extends Monad1<INSTANCE, U>>
    UnaryOperator<FINAL> liftMOp1(
            @NonNull UnaryOperator<U> operator) {
        Objects.requireNonNull(operator, "operator");
        return xM ->
            Type.narrow(xM.flatMap(a -> xM.bless(operator.apply(a))));
    }

    static <U, INSTANCE extends Monad1, FINAL extends Monad1<INSTANCE, U>>
    BinaryOperator<FINAL> liftMOp2(
            @NonNull BinaryOperator<U> operator) {
        Objects.requireNonNull(operator, "operator");
        return (xM, yM) ->
            Type.narrow(xM.flatMap(a -> yM.flatMap(b -> yM.bless(operator.apply(a, b)))));
    }
    ...
    BiFunction<XM, YM, RM> liftMFunc2(
        ...
        Objects.requireNonNull(function, "function");
        return (xM, yM) ->
            Type.narrow(xM.flatMap(x ->
                Type.lF(yM.map(y ->
                    (R) function.apply(x, y)))));
    }
    ...
    Func4<WM, XM, YM, ZM, RM> liftMFunc4(
        ...
        Objects.requireNonNull(function, "function");
        return (wM, xM, yM, zM) ->
            Type.narrow(wM.flatMap(w ->
                xM.flatMap(x ->
                    yM.flatMap(y ->
                        Type.lF(zM.map(z ->
                            (R) function.apply(w, x, y, z)))))));
    }
    ...
    Func5<VM, WM, XM, YM, ZM, RM> liftMFunc5(
        ...
        Objects.requireNonNull(function, "function");
        return (vM, wM, xM, yM, zM) ->
            Type.narrow(vM.flatMap(v ->
                wM.flatMap(w ->
                    xM.flatMap(x ->
                        yM.flatMap(y ->
                            Type.lF(zM.map(z ->
                                (R) function.apply(v, w, x, y, z))))))));
    }
    ...
    Func6<UM, VM, WM, XM, YM, ZM, RM> liftMFunc6(
        ...
        Objects.requireNonNull(function, "function");
        return (uM, vM, wM, xM, yM, zM) ->
            Type.narrow(uM.flatMap(u ->
                vM.flatMap(v ->
                    wM.flatMap(w ->
                        xM.flatMap(x ->
                            yM.flatMap(y ->
                                Type.lF(zM.map(z ->
                                    (R) function.apply(u, v, w, x, y, z)))))))));
    }
    ...
    Func7<TM, UM, VM, WM, XM, YM, ZM, RM> liftMFunc7(
        ...
        Objects.requireNonNull(function, "function");
        return (tM, uM, vM, wM, xM, yM, zM) ->
            Type.narrow(tM.flatMap(t ->
                uM.flatMap(u ->
                    vM.flatMap(v ->
                        wM.flatMap(w ->
                            xM.flatMap(x ->
                                yM.flatMap(y ->
                                    Type.lF(zM.map(z ->
                                        (R) function.apply(t, u, v, w, x, y, z))))))))));
    }
    ...
    Func8<SM, TM, UM, VM, WM, XM, YM, ZM, RM> liftMFunc8(
        ...
        Objects.requireNonNull(function, "function");
        return (sM, tM, uM, vM, wM, xM, yM, zM) ->
            Type.narrow(sM.flatMap(s ->
                tM.flatMap(t ->
                    uM.flatMap(u ->
                        vM.flatMap(v ->
                            wM.flatMap(w ->
                                xM.flatMap(x ->
                                    yM.flatMap(y ->
                                        Type.lF(zM.map(z ->
                                            (R) function.apply(s, t, u, v, w, x, y, z)))))))))));
    }
