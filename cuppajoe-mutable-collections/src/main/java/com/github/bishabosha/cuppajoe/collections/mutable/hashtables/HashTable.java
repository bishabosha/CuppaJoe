...
public class HashTable<E> extends AbstractSet<E> {
    ...
    private HashTable<E> ensureCapacity(int capacity) {
        ...
        if (capacity * LOAD_FACTOR <= MIN_CAPACITY) {
            ...
            newSize = get2PowerLargerThan(LOAD_FACTOR * capacity);
    ...
    private int get2PowerLargerThan(int value) {
        return Stream.iterate(2L, x -> x * 2)
                .filter(t -> t > value)
                .takeWhile(x -> x < Integer.MAX_VALUE)
                .findFirst()
                .map(x -> x.intValue())
