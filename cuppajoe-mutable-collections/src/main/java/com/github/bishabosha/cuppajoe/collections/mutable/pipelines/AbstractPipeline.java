...
class AbstractPipeline<T> extends AbstractBase<T> {
    ...
    private AbstractPipeline() {
        closeHandlers = new com.github.bishabosha.cuppajoe.collections.mutable.lists.LinkedList<>();
        isClosed = false;
    }
    ...
    protected AbstractNode<T, T> getLimit(long maxSize) {
        return ComputeNode.filter(new Predicate<>() {
            long count = 0;
    ...
    public Object[] toArray() {
        return toArrayFunc(List::toArray);
    }
