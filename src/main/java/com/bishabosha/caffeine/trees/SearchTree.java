...
public class SearchTree<E extends Comparable<E>> extends AbstractSet<E> {
	...
	protected E search(E value, Equator<E> equalsTest) {
        current = root;
        while (null != current) {
            if (equalsTest.apply(value, current.value)) {
	...
	protected E replace(E value, Equator<E> replaceTest) {
		...
		current = root;
		while (true) {
			if (replaceTest.apply(value, current.value)) {
				E old = current.value;
	...
	protected E pull(Object o, Equator<E> entryEntryTest) {
		...
		current = root;
		while(current != null) {
			if (entryEntryTest.apply(element, current.value)) {
	...
	private class NodeRemover {
		...
		private void removeNode(boolean hasChild) {
			...
			if (entryEntryTest.apply(current.value, root.value)) {
				root = node;
			} else if (removeLeft) {
				parent.left = node;
			} else {
